...
BinaryReader::BinaryReader(std::iostream* baseStream, size_t alignment, size_t maxSize) : baseStream(baseStream), alignment(alignment), size(maxSize)
{
}
...
void BinaryReader::Seek(SeekOrigin origin, size_t offset)
{
	...
	baseStream->seekg(location, ios::beg);
}
